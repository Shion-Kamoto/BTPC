...
use btpc_core::{
    ...
    crypto::{
        EncryptedWallet, Hash, KeyEntry, PrivateKey, PublicKey, Script, SecurePassword, WalletData,
    },
...
impl Wallet {
    ...
    async fn broadcast_transaction(
        ...
        if let Some(result) = rpc_response.result {
            let txid_hex = result.as_str().ok_or("Invalid txid format from RPC")?;
    ...
    fn load_wallet_data(&mut self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        for key_entry in wallet_data.keys {
            ...
            self.addresses
                .insert(key_entry.address.clone(), wallet_address);
        }

        println!(
            "Wallet loaded successfully. {} addresses found.",
            self.addresses.len()
        );
        Ok(())
    }
    ...
    pub fn restore_wallet(&mut self, backup_path: &str) -> Result<(), Box<dyn std::error::Error>> {
        ...
        println!(
            "Wallet restored successfully. {} addresses recovered.",
            self.addresses.len()
        );
        Ok(())
    }
