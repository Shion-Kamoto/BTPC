...
use btpc_core::{
    ...
    consensus::{pow::MiningTarget, RewardCalculator},
    crypto::Hash,
    Network,
};
...
impl Miner {
    ...
    pub async fn start(&self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        for thread_id in 0..self.config.threads {
            let handle = self
                .start_mining_thread(thread_id, template_cache.clone())
                .await?;
            handles.push(handle);
        }
    ...
    fn create_coinbase_transaction(
        ...
        let coinbase_input = btpc_core::blockchain::TransactionInput {
            ...
            script_sig: btpc_core::crypto::Script::from_bytes(
                config.coinbase_message.as_bytes().to_vec(),
            ),
            sequence: 0xffffffff,
        };
