...
impl std::fmt::Display for ConnectionLimitError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            ConnectionLimitError::SubnetLimitExceeded {
                subnet,
                current,
                limit,
            } => {
                write!(
                    f,
                    "Subnet limit exceeded for {}: {} >= {}",
                    subnet, current, limit
                )
            }
        }
    }
...
mod tests {
    ...
    fn test_subnet_24_limit() {
        ...
        for i in 1..=5 {
            ...
            assert!(
                tracker.can_accept(&addr, &config).is_ok(),
    ...
    fn test_remove_connection() {
        ...
        assert_eq!(
            tracker.connections_from_ip(&"192.168.1.100".parse().unwrap()),
            1
        );
