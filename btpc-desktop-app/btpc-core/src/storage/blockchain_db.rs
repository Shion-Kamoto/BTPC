...
impl BlockchainDatabase for BlockchainDb {
        ...
        let should_update_tip = {
            ...
            let current_tip_height = match self.db.get(b"meta:tip_height") {
                Ok(Some(bytes)) if bytes.len() == 4 => {
                    let height_bytes: [u8; 4] = bytes[0..4]
                        .try_into()
        ...
        match self.db.get(tip_key) {
            Ok(Some(hash_bytes)) => {
                ...
                if hash_bytes.len() != 64 {
                    return Err(BlockchainDbError::InvalidBlockData(format!(
                        "Invalid chain tip hash length: {}",
                        hash_bytes.len()
                    )));
                }
    ...
    fn get_block_height(&self, block_hash: &Hash) -> Result<u32, BlockchainDbError> {
        ...
        match self.db.get(&height_key) {
            Ok(Some(bytes)) => {
                if bytes.len() == 4 {
                    let height_bytes: [u8; 4] = bytes[0..4].try_into().map_err(|_| {
                        BlockchainDbError::InvalidBlockData("Invalid height bytes".to_string())
                    })?;
                    Ok(u32::from_le_bytes(height_bytes))
                } else {
                    Err(BlockchainDbError::InvalidBlockData(
                        "Invalid height length".to_string(),
                    ))
                }
            }
