...
impl RpcServer {
    ...
    where
        F: Fn(Option<serde_json::Value>) -> Result<serde_json::Value, RpcServerError>
            + Send
            + Sync
            + 'static,
    {
    ...
    pub async fn start(&self) -> Result<(), RpcServerError> {
        ...
        println!(
            "DoS Protection: {}conn/IP, {}req/min/IP",
                                    ...
                                    match RpcServer::parse_basic_auth(header) {
                                        Ok((username, password)) => {
                                            ...
                                            if !server.verify_auth(&username, &password) {
                                                ...
                                                {
                                                    eprintln!(
                                                        "Failed to write auth error response: {}",
                                                        e
                                                    );
                                                }
                                                return;
    ...
    pub fn new_validated(config: RpcConfig) -> Result<Self, RpcServerError> {
        ...
        if config.enable_tls {
            config.tls_cert_path.as_ref().ok_or_else(|| {
                RpcServerError::InvalidParams(
                    "TLS enabled but certificate path not provided".to_string(),
                )
            })?;

            config.tls_key_path.as_ref().ok_or_else(|| {
                RpcServerError::InvalidParams(
    ...
    pub fn parse_basic_auth(auth_header: &str) -> Result<(String, String), RpcServerError> {
        ...
        // Decode base64
        let decoded = general_purpose::STANDARD.decode(base64_part).map_err(|_| {
            RpcServerError::InvalidParams("Invalid base64 in auth header".to_string())
        })?;

        let auth_str = String::from_utf8(decoded).map_err(|_| {
            RpcServerError::InvalidParams("Invalid UTF-8 in auth credentials".to_string())
        })?;
    ...
    pub fn verify_auth(&self, provided_username: &str, provided_password: &str) -> bool {
        ...
        let username_match = provided_username
            .as_bytes()
            .ct_eq(config_username.as_bytes());
        let password_match = provided_password
            .as_bytes()
            .ct_eq(config_password.as_bytes());
    ...
    pub fn load_tls_config(config: &RpcConfig) -> Result<Arc<TlsServerConfig>, RpcServerError> {
        ...
        let cert_path = config.tls_cert_path.as_ref().ok_or_else(|| {
            RpcServerError::InvalidParams("TLS certificate path not provided".to_string())
        })?;

        let key_path = config.tls_key_path.as_ref().ok_or_else(|| {
            RpcServerError::InvalidParams("TLS private key path not provided".to_string())
        })?;
        ...
        let certs = rustls_pemfile::certs(&mut cert_reader)
            ...
            .map_err(|e| {
                RpcServerError::InvalidParams(format!("Failed to parse certificate file: {}", e))
            })?;
        ...
        let key = rustls_pemfile::private_key(&mut key_reader)
            .map_err(|e| {
                RpcServerError::InvalidParams(format!("Failed to parse private key file: {}", e))
            })?
            .ok_or_else(|| {
                RpcServerError::InvalidParams(
        ...
        let tls_config = TlsServerConfig::builder()
            ...
            .map_err(|e| {
                RpcServerError::InvalidParams(format!("Failed to create TLS configuration: {}", e))
            })?;
...
mod tests {
    ...
    fn test_server_creation_requires_credentials_when_auth_enabled() {
        ...
        assert!(
            result.is_err(),
            "Server creation should fail when auth enabled but credentials missing"
        );

        let err_msg = result.unwrap_err().to_string();
        assert!(
            err_msg.contains("credentials") || err_msg.contains("Authentication"),
    ...
    fn test_credential_strength_validation_username() {
        ...
        assert!(
            result.is_err(),
            "Server creation should fail with short username"
        );

        let err_msg = result.unwrap_err().to_string();
        assert!(
            err_msg.contains("8 characters") || err_msg.contains("Username"),
    ...
    fn test_credential_strength_validation_password() {
        ...
        assert!(
            result.is_err(),
            "Server creation should fail with short password"
        );

        let err_msg = result.unwrap_err().to_string();
        assert!(
            err_msg.contains("16 characters") || err_msg.contains("Password"),
    ...
    fn test_parse_basic_auth_valid() {
        ...
        assert!(
            result.is_ok(),
            "Should successfully parse valid auth header"
        );
    ...
    fn test_parse_basic_auth_invalid_format() {
        ...
        for header in invalid_headers {
            ...
            assert!(
                result.is_err(),
                "Should reject invalid auth header: {}",
                header
            );
        }
    }
    ...
    fn test_auth_disabled_allows_access() {
        ...
        assert!(
            result.is_ok(),
            "Server creation should succeed when auth is disabled"
        );
    }
    ...
    fn test_tls_config_validation_requires_cert_path() {
        ...
        assert!(
            result.is_err(),
            "Server creation should fail when TLS enabled but cert path missing"
        );

        let err_msg = result.unwrap_err().to_string();
        assert!(
            err_msg.contains("certificate") || err_msg.contains("cert"),
    ...
    fn test_tls_config_validation_requires_key_path() {
        ...
        assert!(
            result.is_err(),
            "Server creation should fail when TLS enabled but key path missing"
        );

        let err_msg = result.unwrap_err().to_string();
        assert!(
            err_msg.contains("key") || err_msg.contains("private"),
    ...
    fn test_load_tls_config_with_valid_files() {
        ...
        writeln!(
            cert_file,
            "MIIBkTCB+wIJAKHHCgVZU6KmMA0GCSqGSIb3DQEBCwUAMBExDzANBgNVBAMMBnRl"
        )
        .unwrap();
        writeln!(
            cert_file,
            "c3RjYTAeFw0yMDA1MTUxNjU4MzlaFw0zMDA1MTMxNjU4MzlaMBExDzANBgNVBAMM"
        )
        .unwrap();
        writeln!(
            cert_file,
            "BnRlc3RjYTBcMA0GCSqGSIb3DQEBAQUAA0sAMEgCQQDQ8bQlCLB8fKxVd5xhUiVb"
        )
        .unwrap();
        writeln!(
            cert_file,
            "QrzFVWnATWFrLBtkx+2llz2bKpvYDjTB0bVAw6gWvEZW/4qW/2vNJ7aNAgMBAAEw"
        )
        .unwrap();
        writeln!(
            cert_file,
            "DQYJKoZIhvcNAQELBQADQQC3AYH6Y3r5h2QhO6kZvQNXvV13Ks4pS5QvYfZlQZZQ"
        )
        .unwrap();
        ...
        writeln!(
            key_file,
            "MIIBVAIBADANBgkqhkiG9w0BAQEFAASCAT4wggE6AgEAAkEA0PG0JQiwfHysVXec"
        )
        .unwrap();
        writeln!(
            key_file,
            "YVIlW0K8xVVpwE1haywbZMftpZc9myqb2A40wdG1QMOoFrxGVv+Klv9rzSe2jQID"
        )
        .unwrap();
        writeln!(
            key_file,
            "AQABAkBXgcXIXGqMUwXqBKJH9yYVlCdFHYFj8yKrNLFqFKjL6VQVMhJzJHQJjQgP"
        )
        .unwrap();
        writeln!(
            key_file,
            "HnAj4mKdSMEG0JpqP3K6JQvhAiEA8eTaWZRjBGxnVQQKGPQXy8pWQJxU3vLnGHFX"
        )
        .unwrap();
        writeln!(
            key_file,
            "BQjYEScCIQDcu/4bPzQxKWdpG4bF/ZQu3rEj5tUQ9qQPYZQcJLLHfwIhAK5x8q8v"
        )
        .unwrap();
        writeln!(
            key_file,
            "kzMuO8E0V+YBwQ0aFqKMnxYZQpVWJLShAiEAoVTQGKQsO3LhZGR5vYp4LVKZqmVY"
        )
        .unwrap();
        writeln!(
            key_file,
            "hQdQqP3P7QcCIABTlPjKNvLMYTOcVxQKYKVqwPvKOc9+gXr7LcfPUHkP"
        )
        .unwrap();
        ...
        assert!(
            result.is_ok(),
            "Should successfully load valid TLS certificate and key files"
        );
    }
    ...
    fn test_load_tls_config_rejects_nonexistent_files() {
        ...
        assert!(
            result.is_err(),
            "Should reject nonexistent certificate/key files"
        );
    ...
    async fn test_tls_and_auth_can_be_combined() {
        ...
        assert!(
            result.is_ok(),
            "Should allow both TLS and authentication to be enabled together"
        );
    }
