...
impl PrivateKey {
    ...
    pub fn sign(&self, data: &[u8]) -> Result<Signature, SignatureError> {
        ...
        Signature::from_bytes(&signature_arr).map_err(|_| SignatureError::SigningFailed)
    }
...
mod tests {
    ...
    fn test_key_pair_bytes_reconstruction() {
        ...
        let reconstructed_key =
            PrivateKey::from_key_pair_bytes(&private_bytes, &public_bytes).unwrap();
    ...
    fn test_private_key_from_bytes_can_sign() {
        ...
        assert!(
            sign_result.is_ok(),
            "T013/T014 FIX: Keys with seed CAN sign!"
        );
