...
impl EncryptedWallet {
    ...
    pub fn encrypt(
        ...
        let plaintext =
            bincode::serialize(wallet_data).map_err(|_| WalletError::SerializationFailed)?;
    ...
    pub fn decrypt(&self, password: &SecurePassword) -> Result<WalletData, WalletError> {
        ...
        let wallet_data =
            bincode::deserialize(&plaintext).map_err(|_| WalletError::DeserializationFailed)?;
    ...
    pub fn save_to_file<P: AsRef<Path>>(&self, path: P) -> Result<(), WalletError> {
        let mut file =
            std::fs::File::create(path).map_err(|e| WalletError::IoError(e.to_string()))?;
    ...
    pub fn load_from_file<P: AsRef<Path>>(path: P) -> Result<Self, WalletError> {
        let mut file =
            std::fs::File::open(path).map_err(|e| WalletError::IoError(e.to_string()))?;
...
impl KeyEntry {
    ...
    pub fn from_private_key(private_key: &PrivateKey, label: String, address: String) -> Self {
        KeyEntry {
            label,
    ...
    pub fn to_public_key(&self) -> Result<PublicKey, WalletError> {
        PublicKey::from_bytes(&self.public_key_bytes).map_err(WalletError::KeyReconstruction)
    }
}
    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            WalletError::DecryptionFailed => {
                write!(
                    f,
                    "Wallet decryption failed (wrong password or corrupted file)"
                )
            }
