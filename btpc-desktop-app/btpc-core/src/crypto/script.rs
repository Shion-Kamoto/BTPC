...
impl Script {
    ...
    pub fn extract_pubkey_hash(&self) -> Option<[u8; 20]> {
        ...
        match &self.operations[..] {
            [ScriptOp::OpDup, ScriptOp::OpHash160, ScriptOp::PushData(hash_data), ScriptOp::OpEqualVerify, ScriptOp::OpCheckMLDSASig] =>
            {
    ...
    fn execute_op(&self, op: &ScriptOp, stack: &mut Vec<Vec<u8>>) -> Result<(), ScriptError> {
        match op {
            ...
            ScriptOp::OpHash160 => {
                ...
                let data = stack.pop().expect("Stack non-empty validated above");
            ...
            ScriptOp::OpVerify => {
                ...
                let value = stack.pop().expect("Stack non-empty validated above");
