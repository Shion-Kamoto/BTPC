...
impl StorageBlockValidator {
    ...
    pub async fn validate_block_with_context(
        ...
        self.validate_coinbase_transaction(block, total_fees)
            .await?;
    ...
    async fn validate_block_context(&self, block: &Block) -> Result<(), StorageValidationError> {
        ...
        if !block.header.prev_hash.is_zero() {
            let blockchain_db = self.blockchain_db.read().map_err(|e| {
                StorageValidationError::LockPoisoned(format!("blockchain_db read lock: {}", e))
            })?;
    ...
    async fn validate_difficulty_adjustment(
        ...
        use crate::consensus::{
            constants::DIFFICULTY_ADJUSTMENT_INTERVAL, difficulty::DifficultyAdjustment,
        };
        ...
        if DifficultyAdjustment::is_adjustment_height(current_height) {
            ...
            self.validate_adjustment_block(block, current_height)
                .await?;
        } else {
    ...
    async fn validate_adjustment_block(
        ...
        let last_block = blockchain_db.get_block(&block.header.prev_hash)?.ok_or(
            StorageValidationError::PreviousBlockNotFound(block.header.prev_hash),
        )?;
        ...
        let expected_target =
            DifficultyAdjustment::adjust_difficulty(&prev_target, actual_timespan, target_timespan);
    ...
    async fn validate_block_transactions(
        ...
        let blockchain_db = self.blockchain_db.read().map_err(|e| {
            StorageValidationError::LockPoisoned(format!(
                "blockchain_db read lock in validate_block_transactions: {}",
                e
            ))
        })?;
    ...
    async fn validate_transaction_with_utxos(
        ...
        let blockchain_db = self.blockchain_db.read().map_err(|e| {
            StorageValidationError::LockPoisoned(format!(
                "blockchain_db read lock in validate_transaction_with_utxos: {}",
                e
            ))
        })?;
        ...
        let utxo_db = self.utxo_db.read().map_err(|e| {
            StorageValidationError::LockPoisoned(format!(
                "utxo_db read lock in validate_transaction_with_utxos: {}",
                e
            ))
        })?;
    ...
    async fn get_block_height(&self, block: &Block) -> Result<u32, StorageValidationError> {
        ...
        let blockchain_db = self.blockchain_db.read().map_err(|e| {
            StorageValidationError::LockPoisoned(format!(
                "blockchain_db read lock in get_block_height: {}",
                e
            ))
        })?;
        loop {
    ...
    async fn get_previous_blocks(
        ...
        let blockchain_db = self.blockchain_db.read().map_err(|e| {
            StorageValidationError::LockPoisoned(format!(
                "blockchain_db read lock in get_previous_blocks: {}",
                e
            ))
        })?;
        for _ in 0..count {
    ...
    pub async fn apply_block(&self, block: &Block) -> Result<(), StorageValidationError> {
        ...
        {
            let mut utxo_db = self.utxo_db.write().map_err(|e| {
                StorageValidationError::LockPoisoned(format!(
                    "utxo_db write lock in apply_block: {}",
                    e
                ))
            })?;
        ...
        let mut blockchain_db = self.blockchain_db.write().map_err(|e| {
            StorageValidationError::LockPoisoned(format!(
                "blockchain_db write lock in apply_block: {}",
                e
            ))
        })?;
    ...
    async fn apply_transaction(
        ...
        let mut utxo_db = self.utxo_db.write().map_err(|e| {
            StorageValidationError::LockPoisoned(format!(
                "utxo_db write lock in apply_transaction: {}",
                e
            ))
        })?;
    ...
    pub async fn get_chain_tip(&self) -> Result<Option<Block>, StorageValidationError> {
        let blockchain_db = self.blockchain_db.read().map_err(|e| {
            StorageValidationError::LockPoisoned(format!(
                "blockchain_db read lock in get_chain_tip: {}",
                e
            ))
        })?;
    ...
    pub async fn has_block(&self, block_hash: &Hash) -> Result<bool, StorageValidationError> {
        let blockchain_db = self.blockchain_db.read().map_err(|e| {
            StorageValidationError::LockPoisoned(format!(
                "blockchain_db read lock in has_block: {}",
                e
            ))
        })?;
...
impl StorageTransactionValidator {
    ...
    async fn validate_transaction_inputs(
        ...
        let utxo_db = self.utxo_db.read().map_err(|e| {
            StorageValidationError::LockPoisoned(format!(
                "utxo_db read lock in validate_transaction_inputs: {}",
                e
            ))
        })?;
...
pub enum StorageValidationError {
    ...
    BlockMinedTooSoon { time_since_prev: u64, min_time: u64 },
...
mod tests {
    ...
    async fn test_storage_validator_creation() {
        ...
        use crate::consensus::pow::{MiningTarget, ProofOfWork};
    ...
    async fn test_valid_signature_acceptance() {
        ...
        let result = validator
            .validate_transaction_with_utxos(&transaction)
            .await;
        assert!(result.is_ok());
