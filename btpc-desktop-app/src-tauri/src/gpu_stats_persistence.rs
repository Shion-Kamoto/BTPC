...
impl GpuStatsPersistence {
    ...
    pub fn load(&self) -> GpuStatsFile {
        match self.load_internal() {
            ...
            Err(e) => {
                eprintln!(
                    "Warning: Failed to load GPU stats ({}), using empty state",
                    e
                );
    ...
    fn load_internal(&self) -> Result<GpuStatsFile> {
        ...
        let contents =
            fs::read_to_string(&self.file_path).context("Failed to read GPU stats file")?;

        let stats: GpuStatsFile =
            serde_json::from_str(&contents).context("Failed to parse GPU stats JSON")?;
    ...
    pub fn save(&self, stats: &GpuStatsFile) -> Result<()> {
        ...
        if let Some(parent) = self.file_path.parent() {
            fs::create_dir_all(parent).context("Failed to create GPU stats directory")?;
        }

        // Write to temp file
        let temp_path = self.file_path.with_extension("tmp");
        let json = serde_json::to_string_pretty(stats).context("Failed to serialize GPU stats")?;

        fs::write(&temp_path, json).context("Failed to write GPU stats temp file")?;

        // Atomic rename
        fs::rename(&temp_path, &self.file_path).context("Failed to rename GPU stats temp file")?;
    ...
    pub fn update_gpu_stats(
        ...
        let entry = stats_file
            .gpus
            .entry(device_index)
            .or_insert_with(|| PersistentGpuStats {
                device_index,
