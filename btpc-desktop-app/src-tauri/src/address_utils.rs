...
fn find_address_by_public_key(
    ...
    for entry in wallet_files {
        ...
        if path
            .file_name()
            .is_some_and(|name| name == "wallets_metadata.json")
        {
            continue;
        }
                ...
                if let Some(pk) = wallet_data.get("public_key").and_then(|v| v.as_str()) {
                    if pk == public_key_hex {
                        ...
                        if let Some(address) = wallet_data.get("address").and_then(|v| v.as_str()) {
                            println!(
                                "✅ Converted {} char public key to Base58: {}",
                                public_key_hex.len(),
                                address
                            );
    ...
    Err(format!(
        "No wallet found with public key: {}...",
        &public_key_hex[..20]
    ))
}
...
pub fn migrate_utxo_addresses(utxo_file: &PathBuf, wallets_dir: &PathBuf) -> Result<usize, String> {
    // Read UTXO file
    if !utxo_file.exists() {
    ...
    let mut utxos: Vec<Value> =
        serde_json::from_str(&content).map_err(|e| format!("Failed to parse UTXO file: {}", e))?;
            ...
            if address_owned.len() > 128 && address_owned.chars().all(|c| c.is_ascii_hexdigit()) {
                match find_address_by_public_key(&address_owned, wallets_dir) {
                    Ok(base58_address) => {
                        ...
                        migration_count += 1;
                        println!(
                            "Migrated UTXO {} from {} char hex to Base58: {}",
                            utxo.get("txid")
                                .and_then(|v| v.as_str())
                                .unwrap_or("unknown"),
                            address_owned.len(),
    ...
    if migration_count > 0 {
        ...
        println!(
            "✅ Migrated {} UTXO addresses to Base58 format",
            migration_count
        );
        if !errors.is_empty() {
