...
pub async fn get_gpu_stats(state: State<'_, crate::AppState>) -> Result<GpuStats, String> {
    ...
    let device_info = pool.get_gpu_device_info(0).ok_or_else(|| {
        "GPU device 0 not found - mining may not have initialized properly".to_string()
    })?;
...
pub async fn get_gpu_mining_stats(
    ...
    let per_gpu_stats = if let Some(device_index) = gpu_device_index {
        ...
        match pool.get_gpu_stats(device_index) {
            ...
            None => {
                return Err(format!(
                    "GPU device {} not found or not mining",
                    device_index
                ));
            }
        }
...
pub async fn get_gpu_health_metrics(
    ...
    if let Some(device_index) = gpu_device_index {
        ...
        if metrics.is_empty() {
            return Err(
                "No GPU health data available - GPUs may not be detected or sensors unavailable"
                    .to_string(),
            );
        }
...
#[command]
pub async fn get_temperature_threshold(state: State<'_, crate::AppState>) -> Result<f32, String> {
    let threshold = state.gpu_temperature_threshold.read().await;
    Ok(*threshold)
}
...
mod tests {
    ...
    fn test_gpu_stats_serde() {
        ...
        // Deserialize back
        let deserialized: GpuStats = serde_json::from_str(&json).expect("Failed to deserialize");
