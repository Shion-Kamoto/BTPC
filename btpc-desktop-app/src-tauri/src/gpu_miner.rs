...
impl GpuMiner {
    ...
    pub fn new(device_index: u32) -> Result<Self> {
        ...
        let platforms =
            get_platforms().map_err(|e| anyhow!("Failed to get OpenCL platforms: {}", e))?;
        ...
        let program =
            match Program::create_and_build_from_source(&context, KERNEL_SOURCE, BUILD_OPTIONS) {
                Ok(p) => p,
                Err(e) => {
                                ...
                                Err(build_err) => {
                                    ...
                                    match prog.get_build_log(device.id()) {
                                        ...
                                        Err(log_err) => {
                                            ...
                                            return Err(anyhow!(
                                                "OpenCL kernel build failed: {}, log error: {}",
                                                build_err,
                                                log_err
                                            ));
                                        }
    ...
    pub fn mine_batch(
        ...
        unsafe {
            self.queue
                .enqueue_read_buffer(&self.result_buffer, CL_BLOCKING, 0, &mut result, &[])
                .map_err(|e| anyhow!("Failed to read result: {}", e))?;
        }

        // Update hash count
        self.hashes_computed
            .fetch_add(NONCES_PER_BATCH as u64, Ordering::Relaxed);
...
pub fn enumerate_gpu_devices() -> Result<Vec<GpuDevice>> {
        ...
        let device_ids = match platform.get_devices(CL_DEVICE_TYPE_GPU) {
            ...
            _ => {
                ...
                match platform.get_devices(CL_DEVICE_TYPE_ALL) {
                    Ok(ids) if !ids.is_empty() => {
                        eprintln!(
                            "    ⚠️ GPU query empty, CL_DEVICE_TYPE_ALL found {} device(s)",
                            ids.len()
                        );
                        ids
                    }
    ...
    eprintln!(
        "🎮 Found {} total GPU device(s) across all platforms",
        all_devices.len()
    );
