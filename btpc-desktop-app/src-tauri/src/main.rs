...
pub mod error;
mod wallet_commands;
mod wallet_manager; // Public for integration tests
...
mod transaction_monitor; // Event types for Article XI compliance
...
pub mod process_health; // Health monitoring and crash recovery (FR-038 through FR-046) // Safe file locking using fs2 (replaces unsafe libc::flock)

// Feature 013: Embedded Blockchain Node Commands (self-contained, no external btpc_node process)
mod commands; // Tauri commands directory (includes embedded_node commands)

use error::{BtpcError, BtpcResult};

use address_book::{
    AddAddressBookRequest, AddressBookEntry, AddressBookManager, UpdateAddressBookRequest,
};
use btpc_desktop_app::utxo_manager::{UTXOManager, UTXOStats, UTXO};
use btpc_integration::BtpcIntegration;
use security::{RecoveryData, SecurityManager, UserSession};
...
pub struct AppState {
    ...
    utxo_manager: Arc<Mutex<UTXOManager>>, // Using std::sync::Mutex for simplicity
    wallet_manager: Arc<Mutex<WalletManager>>, // Using std::sync::Mutex for simplicity
    ...
    embedded_node: Arc<RwLock<btpc_desktop_app::embedded_node::EmbeddedNode>>, // In-process blockchain node
}
...
impl AppState {
    pub fn new() -> BtpcResult<Self> {
        ...
        let app_state = Self {
            ...
            utxo_manager: utxo_manager_arc.clone(), // Use the shared Arc<Mutex<>> instance
            wallet_manager: Arc::new(Mutex::new(wallet_manager)), // Using std::sync::Mutex
            ...
            // Feature 013: Embedded Blockchain Node (self-contained, no external btpc_node process)
            embedded_node, // In-process blockchain node (already Arc<RwLock<EmbeddedNode>>)
        };
...
#[tauri::command]
async fn get_transaction_count_from_storage(state: State<'_, AppState>) -> Result<usize, String> {
    // Get wallet address from WalletManager
    let address = {
        let wallet_manager = state
            .wallet_manager
...
#[tauri::command]
async fn check_wallet_lock_status(state: State<'_, AppState>) -> Result<bool, String> {
...
#[tauri::command]
async fn lock_wallets(state: State<'_, AppState>) -> Result<String, String> {
...
async fn get_blockchain_info(state: State<'_, AppState>) -> Result<serde_json::Value, String> {
    ...
    Ok(serde_json::json!({
        ...
        "chain": network,
        "difficulty": 1.0,  // Placeholder - will be implemented in consensus module
...
#[tauri::command]
async fn get_block_message(state: State<'_, AppState>, txid: String) -> Result<String, String> {
