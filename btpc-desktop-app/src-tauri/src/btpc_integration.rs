...
use btpc_core::crypto::{
    Address, EncryptedWallet, KeyEntry, PrivateKey, SecurePassword, WalletData,
};
...
impl BtpcIntegration {
    ...
    pub fn check_installation(&self) -> BtpcInstallationStatus {
        let required_binaries = ["btpc-quantum-resistant-chain", "btpc_wallet_dilithium"];

        let optional_binaries = ["btpc_secure_wallet", "btpc_miner", "integrated_mining_demo"];
    ...
    pub fn get_wallet_balance(&self, wallet_file: &Path) -> Result<String> {
        ...
        let output = self.execute_binary(
            "btpc_wallet",
            &["balance", "--file", &wallet_file.display().to_string()],
        )?;
