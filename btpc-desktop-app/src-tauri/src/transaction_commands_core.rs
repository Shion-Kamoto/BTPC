...
use btpc_core::crypto::{Address, EncryptedWallet, Script, SecurePassword};
...
pub fn create_transaction_core(
    ...
    // Validate addresses
    Address::from_string(from_address).map_err(|e| TransactionError::InvalidAddress {
        address: from_address.to_string(),
        reason: e.to_string(),
    })?;

    Address::from_string(to_address).map_err(|e| TransactionError::InvalidAddress {
...
pub fn estimate_fee_core(
    ...
    if utxos.is_empty() {
        return Err(TransactionError::UTXOSelectionFailed(
...
pub async fn broadcast_transaction_core(
    ...
    if !all_signed {
        ...
        return Err(TransactionError::Other(
            "Transaction not fully signed - cannot broadcast".to_string(),
        ));
    }
    ...
    if !rpc_client.ping().await.unwrap_or(false) {
        ...
        return Err(TransactionError::Other(
            "Cannot connect to BTPC node - is the node running?".to_string(),
        ));
    }
...
pub fn cancel_transaction_core(
    ...
    // Get current state
    let tx_status = tx_state_manager.get_state(transaction_id).ok_or_else(|| {
        TransactionError::Other(format!("Transaction {} not found", transaction_id))
    })?;
    ...
    {
        return Err(TransactionError::Other(
            "Cannot cancel transaction that has been broadcast".to_string(),
        ));
    }
...
pub fn sign_transaction_core(
    ...
    let wallet_data = encrypted_wallet.decrypt(&secure_password).map_err(|e| {
        TransactionError::Other(format!("Failed to decrypt wallet (wrong password?): {}", e))
    })?;
...
fn validate_wallet_integrity(
    ...
    if wallet_data.wallet_id.is_empty() {
        return Err(TransactionError::Other(
            "Wallet corruption detected: wallet_id field is empty".to_string(),
        ));
    }
    ...
    if wallet_data.network.is_empty() {
        return Err(TransactionError::Other(
            "Wallet corruption detected: network field is missing or empty".to_string(),
        ));
    }
    ...
    if wallet_data.created_at == 0 || wallet_data.modified_at == 0 {
        return Err(TransactionError::Other(
            "Wallet corruption detected: invalid timestamps (zero values)".to_string(),
        ));
    }
...
mod tests {
    ...
    fn test_cancel_transaction_core_already_broadcast() {
        ...
        assert!(
            result.is_err(),
            "Should fail to cancel broadcast transaction"
        );

        let err = result.unwrap_err();
        assert!(err
            .to_string()
            .contains("Cannot cancel transaction that has been broadcast"));
    }
    ...
    fn test_cancel_transaction_core_already_confirmed() {
        ...
        assert!(
            result.is_err(),
            "Should fail to cancel confirmed transaction"
        );
    ...
    fn test_cancel_transaction_core_with_reservation() {
        ...
        assert!(
            result.is_ok(),
            "Should successfully cancel and return state with reservation info"
        );
    ...
    fn test_broadcast_transaction_core_unsigned_transaction() {
        ...
        assert!(
            result.is_err(),
            "Should fail to broadcast unsigned transaction"
        );
    ...
    fn test_broadcast_transaction_core_rpc_connection_failed() {
        ...
        assert!(
            result.is_err(),
            "Should fail when RPC node is not available"
        );
    ...
    fn test_serialize_transaction_to_bytes() {
        ...
        assert!(
            bytes.len() > 10,
            "Serialized transaction should have reasonable size"
        );
    }
