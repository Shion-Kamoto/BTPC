...
fn test_wallet_creation_uses_argon2id() {
    ...
    // Create wallet (use Regtest network for tests)
    // Returns: (address, seed_phrase, private_key_hex, seed_hex)
    let (address, seed_phrase, private_key_hex, _seed_hex) = integration
        .create_wallet(&wallet_file, password, btpc_core::Network::Regtest)
    ...
    let encrypted_wallet =
        EncryptedWallet::load_from_file(&wallet_dat_file).expect("Failed to load encrypted wallet");
    ...
    assert!(
        ...
        "Address should be Base58 format"
    );
    ...
    assert_eq!(
        ...
        24,
        "Should have 24-word mnemonic"
    );

    // Verify private key hex (ML-DSA-87 / Dilithium5 mode5 secret key = 4864 bytes = 9728 hex chars)
    assert_eq!(
        private_key_hex.len(),
        9728,
        "Private key should be 4864 bytes (9728 hex)"
    );
}
...
fn test_wallet_decryption_with_correct_password() {
    ...
    // Create wallet (use Regtest network for tests)
    // Returns: (address, seed_phrase, private_key_hex, seed_hex)
    let (original_address, _, original_key_hex, _) = integration
        .create_wallet(&wallet_file, password, btpc_core::Network::Regtest)
        .expect("Wallet creation failed");
    ...
    assert!(
        !key_entry.private_key_bytes.is_empty(),
...
fn test_wallet_decryption_with_wrong_password() {
    ...
    // Create wallet with correct password (use Regtest network for tests)
    integration
        .create_wallet(&wallet_file, correct_password, btpc_core::Network::Regtest)
        .expect("Wallet creation failed");
...
fn test_migration_from_sha256_to_argon2id() {
    ...
    // Create directory
    std::fs::create_dir_all(wallet_file.parent().unwrap()).expect("Failed to create wallet dir");
    ...
    std::fs::write(
        &wallet_file,
        serde_json::to_string_pretty(&legacy_wallet).unwrap(),
    )
    ...
    let is_legacy = integration
        .is_legacy_wallet_format(&wallet_file)
