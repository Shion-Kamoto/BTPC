...
pub fn clean_orphaned_utxos(
    ...
    println!(
        "📊 Found {} wallet identifiers (public keys + addresses)",
        wallet_identifiers.len()
    );
    ...
    println!(
        "  ✅ Owned UTXOs (belong to current wallets): {}",
        report.owned_utxos
    );
    println!(
        "  ❌ Orphaned UTXOs (no matching wallet): {}",
        report.orphaned_utxos
    );
    println!(
        "  💰 Orphaned value: {} credits ({:.8} BTP)",
    ...
    if !dry_run && report.orphaned_utxos > 0 {
        ...
        println!(
            "✅ Removed {} orphaned UTXOs from {}",
            report.orphaned_utxos,
            utxo_file.display()
        );
        println!(
            "⚠️  WARNING: Removed {} BTP ({} credits) in orphaned funds",
            report.orphaned_value_btp, report.orphaned_value_credits
        );
    } else if report.orphaned_utxos > 0 {
        println!(
            "\n🔍 DRY RUN: Would remove {} orphaned UTXOs",
            report.orphaned_utxos
        );
