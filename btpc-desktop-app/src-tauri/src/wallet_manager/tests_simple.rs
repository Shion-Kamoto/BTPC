...
mod tests {
    ...
    fn setup_test_environment() -> (TempDir, WalletManager, BtpcIntegration) {
        ...
        let wallet_manager =
            WalletManager::new(config, security).expect("Failed to create wallet manager");
    ...
    fn test_create_wallet_with_new_api() {
        ...
        let result = wallet_manager.create_wallet(request, &btpc, btpc_core::Network::Regtest);
        ...
        assert!(
            !response.private_key_hex.is_empty(),
    ...
    fn test_wallet_retrieval() {
        ...
        let response = wallet_manager.create_wallet(request, &btpc, btpc_core::Network::Regtest).unwrap();
    ...
    fn test_encrypted_wallet_persistence() {
        ...
        let wallet_response = wallet_manager.create_wallet(request, &btpc, btpc_core::Network::Regtest).unwrap();
    ...
    fn test_encrypted_wallet_wrong_password() {
        ...
        wallet_manager.create_wallet(request, &btpc, btpc_core::Network::Regtest).unwrap();

        let correct_password = SecurePassword::new("correct_pass".to_string());
        wallet_manager
            .save_wallets_encrypted(&correct_password)
            .unwrap();
