...
impl fmt::Display for IntegrationError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            IntegrationError::BinaryNotFound {
                binary_name,
                expected_path,
            } => {
                write!(
            ...
            IntegrationError::ExecutionTimeout {
                binary_name,
                timeout_secs,
            } => {
                write!(
                    ...
                    binary_name, timeout_secs
                )
            }
            IntegrationError::IncompatibleVersion {
                binary_name,
                version,
                required,
            } => {
                write!(
                    f,
                    "Binary '{}' version {} is incompatible (required: {})",
                    binary_name, version, required
                )
            }
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            UtxoError::InsufficientFunds {
                ...
                write!(
                    f,
                    "Insufficient funds: available {} credits, required {} credits",
                    available, required
                )
            }
...
impl fmt::Display for ProcessError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            ProcessError::MutexPoisoned {
                component,
                operation,
            } => {
                write!(
                    f,
                    "Internal error: {} mutex poisoned during {}",
                    component, operation
                )
            }
            ProcessError::DatabaseLocked {
                database,
                holder_info,
            } => {
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ValidationError::FieldValidation {
                ...
                write!(
                    f,
                    "Field '{}' with value '{}' violates constraint: {}",
            ...
            ValidationError::RangeValidation {
                field,
                value,
                min,
                max,
...
impl fmt::Display for TransactionError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            TransactionError::SeedMissing {
                wallet_id,
                key_index,
            } => {
                write!(
                    f,
                    "Seed missing for wallet {} key index {}. Restore from backup.",
                    wallet_id, key_index
                )
            }
            TransactionError::SignatureFailed {
                input_index,
                reason,
            } => {
            ...
            TransactionError::WalletLocked { wallet_id } => {
                write!(
                    f,
                    "Wallet {} is locked. Enter password to unlock.",
                    wallet_id
                )
            }
            ...
            TransactionError::MempoolFull => {
                write!(
                    f,
                    "Mempool is full. Transaction queued for later broadcast."
                )
            }
...
impl From<std::io::Error> for BtpcError {
    fn from(err: std::io::Error) -> Self {
        match err.kind() {
            std::io::ErrorKind::NotFound => BtpcError::FileSystem(FileSystemError::FileNotFound {
...
impl ErrorSanitization for BtpcError {
    fn user_message(&self) -> String {
        match self {
            ...
            BtpcError::Process(ProcessError::MutexPoisoned { component, .. }) => {
                format!(
                    "Internal error in {}. Please restart the application",
                    component
                )
            }
    ...
    fn technical_details(&self) -> Option<String> {
        ...
        let sanitized = details.replace(|c: char| c.is_ascii_hexdigit() && details.len() > 64, "X");
