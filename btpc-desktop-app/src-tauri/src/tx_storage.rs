...
impl TransactionStorage {
    ...
    pub fn open<P: AsRef<Path>>(path: P) -> Result<Self> {
        ...
        let db =
            DB::open_cf(&opts, path, &cfs).map_err(|e| anyhow!("Failed to open RocksDB: {}", e))?;
    ...
    pub fn mark_utxo_spent(
        ...
        if let Some(utxo_bytes) = self.db.get_cf(&cf_utxos, utxo_key.as_bytes())? {
            ...
            self.db
                .put_cf(&cf_utxos, utxo_key.as_bytes(), updated_bytes)?;
            Ok(())
        } else {
    ...
    pub fn get_transactions_for_address(
        ...
        for txid in paginated_ids {
            if let Some(tx_bytes) = self.db.get_cf(&cf_tx, txid.as_bytes())? {
                ...
                let outputs_with_address: Vec<OutputWithAddress> = tx
                    .outputs
                    .iter()
                    .map(|out| {
                        OutputWithAddress {
