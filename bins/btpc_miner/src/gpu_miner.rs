...
use btpc_core::{blockchain::BlockHeader, consensus::pow::MiningTarget};
use std::sync::{
    ...
    pub fn new(config: GpuMinerConfig) -> Result<Self, String> {
        #[cfg(feature = "gpu")]
        {
            ...
            if config.platform_id >= platforms.len() {
                return Err(format!(
                    "Invalid platform_id: {} (only {} platforms available)",
                    config.platform_id,
                    platforms.len()
                ));
            }
            ...
            if config.device_id >= devices.len() {
                return Err(format!(
                    "Invalid device_id: {} (only {} devices available)",
                    config.device_id,
                    devices.len()
                ));
            }
...
mod tests {
    use super::*;
    use btpc_core::{blockchain::BlockHeader, consensus::pow::MiningTarget, crypto::Hash};
