...
use btpc_desktop_app::utxo_manager::{Transaction, TxInput, TxOutput, UTXOManager, UTXO};
use chrono::Utc;
...
pub struct BlockchainSyncService {
    ...
    /// Network fork ID for replay protection (0=mainnet, 1=testnet, 2=regtest)
    fork_id: u8,
}
...
impl BlockchainSyncService {
    ...
    async fn sync_iteration(
        ...
        for height in (current_height + 1)..=(current_height + blocks_to_sync) {
            match Self::sync_block(rpc_client, utxo_manager, height, app, fork_id).await {
                Ok(_) => {
                    synced_count += 1;
        ...
        for (tx_index, txid) in block_info.tx.iter().enumerate() {
            ...
            for output in &tx_info.vout {
                ...
                let utxo = UTXO {
                    ...
                    script_pubkey: hex::decode(&output.script_pub_key.hex).unwrap_or_default(),
                };
            ...
            if !is_coinbase {
                for input in &tx_info.vin {
                    let mut utxo_manager_guard = utxo_manager.lock().unwrap();
                    if let Err(e) =
                        utxo_manager_guard.spend_utxo(&input.txid, input.vout, txid.clone(), height)
                    {
            ...
            let transaction = Transaction {
                ...
                fork_id, // Network-specific (passed to sync_block)
                block_height: Some(height),
                confirmed_at: Some(Utc::now()),
                is_coinbase,
                // Synced transactions don't have sender_address - determined from inputs at query time
                sender_address: None,
            };
...
mod tests {
    ...
    async fn test_sync_service_creation() {
        ...
        let sync_service = BlockchainSyncService::new(utxo_manager, SyncConfig::default(), None, 2);
