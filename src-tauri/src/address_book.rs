...
impl AddressBookManager {
    ...
    pub fn new(storage_dir: PathBuf) -> BtpcResult<Self> {
        ...
        std::fs::create_dir_all(&storage_dir).map_err(|_| {
        ...
        if Address::from_string(&request.address).is_err() {
            return Err(BtpcError::Validation(
                crate::error::ValidationError::CustomValidation {
                    ...
                    message:
                        "Invalid BTPC address format (must be valid Base58 address with checksum)"
                            .to_string(),
                },
            ));
        }
    ...
    pub fn list_entries(&self) -> Vec<AddressBookEntry> {
        ...
        entries.sort_by(|a, b| match (a.last_used, b.last_used) {
    ...
    ) -> BtpcResult<AddressBookEntry> {
        let entry = self.entries.get_mut(&request.id).ok_or_else(|| {
    ...
    fn load_entries(&mut self) -> BtpcResult<()> {
        ...
        let content = std::fs::read_to_string(&self.storage_file).map_err(|_| {
        ...
        self.entries = serde_json::from_str(&content).map_err(|_| {
    ...
    fn save_entries(&self) -> BtpcResult<()> {
        let content = serde_json::to_string_pretty(&self.entries).map_err(|_| {
            BtpcError::Utxo(crate::error::UtxoError::SerializationError {
                data_type: "address book".to_string(),
            })
        })?;

        std::fs::write(&self.storage_file, content).map_err(|_| {
