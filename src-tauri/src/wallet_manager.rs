...
use btpc_core::crypto::{EncryptedWallet, KeyEntry, SecurePassword, WalletData};
...
impl WalletManager {
    ...
    pub fn new(config: WalletManagerConfig, security: SecurityManager) -> BtpcResult<Self> {
        ...
        std::fs::create_dir_all(&config.wallets_dir).map_err(|_e| {
            BtpcError::FileSystem(crate::error::FileSystemError::DirectoryCreationFailed {
                path: config.wallets_dir.display().to_string(),
            })
        })?;

        std::fs::create_dir_all(&config.backups_dir).map_err(|_e| {
            BtpcError::FileSystem(crate::error::FileSystemError::DirectoryCreationFailed {
    ...
    pub fn delete_wallet(&mut self, wallet_id: &str) -> BtpcResult<()> {
        let wallet = self.wallets.get(wallet_id).ok_or_else(|| {
            BtpcError::Validation(crate::error::ValidationError::CustomValidation {
                rule: "wallet_exists".to_string(),
        ...
        if wallet.file_path.exists() {
            std::fs::remove_file(&wallet.file_path).map_err(|_| {
                BtpcError::FileSystem(crate::error::FileSystemError::WriteFailed {
    ...
    pub fn update_wallet_balance(
        ...
        let wallet = self.wallets.get_mut(wallet_id).ok_or_else(|| {
            BtpcError::Validation(crate::error::ValidationError::CustomValidation {
                rule: "wallet_exists".to_string(),
                message: format!("Wallet with ID '{}' not found", wallet_id),
            })
    ...
    pub fn get_summary(&self) -> WalletSummary {
        let total_balance_credits: u64 = self
            .wallets
            .values()
    ...
    pub fn backup_wallet(&self, wallet_id: &str) -> BtpcResult<PathBuf> {
        let wallet = self.get_wallet(wallet_id).ok_or_else(|| {
            BtpcError::Validation(crate::error::ValidationError::CustomValidation {
                rule: "wallet_exists".to_string(),
                message: format!("Wallet with ID '{}' not found", wallet_id),
            })
        ...
        if !self.config.backups_dir.exists() {
            std::fs::create_dir_all(&self.config.backups_dir).map_err(|e| {
                BtpcError::FileSystem(crate::error::FileSystemError::WriteFailed {
        ...
        if !wallet.file_path.exists() {
            return Err(BtpcError::FileSystem(
                crate::error::FileSystemError::ReadFailed {
                    path: wallet.file_path.display().to_string(),
        ...
        std::fs::copy(&wallet.file_path, &backup_path).map_err(|e| {
            BtpcError::FileSystem(crate::error::FileSystemError::WriteFailed {
    ...
    fn load_wallets(&mut self) -> BtpcResult<()> {
        ...
        let content = std::fs::read_to_string(&self.metadata_file).map_err(|_| {
            BtpcError::FileSystem(crate::error::FileSystemError::ReadFailed {
    ...
    fn save_wallets(&self) -> BtpcResult<()> {
        let content = serde_json::to_string_pretty(&self.wallets).map_err(|_| {
            BtpcError::Utxo(crate::error::UtxoError::SerializationError {
                data_type: "wallet metadata".to_string(),
            })
        })?;

        std::fs::write(&self.metadata_file, content).map_err(|_| {
            BtpcError::FileSystem(crate::error::FileSystemError::WriteFailed {
                path: self.metadata_file.display().to_string(),
    ...
    pub fn save_wallets_encrypted(&self, password: &SecurePassword) -> BtpcResult<()> {
        ...
        let metadata_json = serde_json::to_string(&self.wallets).map_err(|_| {
            BtpcError::Utxo(crate::error::UtxoError::SerializationError {
                data_type: "wallet metadata".to_string(),
            })
        })?;
        ...
        // Encrypt wallet data
        let encrypted = EncryptedWallet::encrypt(&wallet_data, password).map_err(|e| {
            BtpcError::FileSystem(crate::error::FileSystemError::WriteFailed {
                path: self.metadata_file.display().to_string(),
        ...
        encrypted.save_to_file(&encrypted_path).map_err(|e| {
            BtpcError::FileSystem(crate::error::FileSystemError::WriteFailed {
    ...
    pub fn load_wallets_encrypted(&mut self, password: &SecurePassword) -> BtpcResult<()> {
        ...
        let encrypted = EncryptedWallet::load_from_file(&encrypted_path).map_err(|e| {
            BtpcError::FileSystem(crate::error::FileSystemError::ReadFailed {
                path: encrypted_path.display().to_string(),
        ...
        // Decrypt wallet data
        let wallet_data = encrypted.decrypt(password).map_err(|e| {
            BtpcError::FileSystem(crate::error::FileSystemError::ReadFailed {
                path: encrypted_path.display().to_string(),
        ...
        if let Some(metadata_entry) = wallet_data.keys.first() {
            if metadata_entry.label == "wallet_metadata" {
                ...
                self.wallets = serde_json::from_str(&metadata_entry.address).map_err(|_| {
                    BtpcError::Utxo(crate::error::UtxoError::DeserializationError {
                        data_type: "wallet metadata".to_string(),
    ...
    pub fn release_reservation(&self, token: &ReservationToken) -> BtpcResult<usize> {
        ...
        if reservations.remove(&token.id).is_some() {
            tracing::info!("Released reservation {} ({} UTXOs)", token.id, utxo_count);
            Ok(utxo_count)
        } else {
    ...
    pub fn is_utxo_reserved(&self, wallet_id: &str, txid: &str, vout: u32) -> BtpcResult<bool> {
        ...
        let is_reserved = reservations.values().any(|token| {
            token.wallet_id == wallet_id && !token.is_expired() && token.contains_utxo(txid, vout)
        });
